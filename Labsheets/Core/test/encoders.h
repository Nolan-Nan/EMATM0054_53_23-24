...
ISR( INT6_vect ) {
  ...
  state_right = state_right | (right_A << 2);
    ...
    if( state_right == 1 || state_right == 7 || state_right == 8 || state_right == 14 ) {
      count_right += 1; //forward
    } else if( state_right == 2 || state_right == 4 || state_right == 11 || state_right == 13) {
...
ISR( PCINT0_vect ) {
    ...
    //boolean state_right_B = PINE & B00000100;  // Does same as above.
    ...
    // Handle which transition we have registered.
    if( state_left == 1 || state_left == 7 || state_left == 8 || state_left == 14 ) {
      count_left += 1; //forward
    }else if( state_left == 2 || state_left == 4 || state_left == 11 || state_left == 13) {
...
void setupEncoder0() {
    ...
    // initialise the recorded state of state_right encoder.
    state_right = 0;

    // Get initial state of encoder pins A + B
    boolean state_right_A = digitalRead( ENCODER_0_A_PIN );
    boolean state_right_B = digitalRead( ENCODER_0_B_PIN );
    state_right_A = state_right_A ^ state_right_B;

    // Shift values into correct place in state.
    // Bits 1 and 0  are prior states.
    state_right = state_right | ( state_right_B << 1 );
    state_right = state_right | ( state_right_A << 0 );
